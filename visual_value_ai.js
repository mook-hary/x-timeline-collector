...
const { readJsonRequired, readJsonObjectOptional, writeJsonAtomic } = require('./lib/pipeline-io');
...
async function main(argv = process.argv.slice(2)) {
  ...
  const input = readJsonRequired(resolve('input', 'daily-vision.json'));
  ...
  if (!dryRun) {
    writeJsonAtomic(resolve('output', 'daily-visual.json'), Array.isArray(input) ? result.posts : { ...input, posts: result.posts, itemCount: result.posts.length, visualValueStage: result.summary });
