...
Platform Launcher は Launcher（4173）・Editorial（4174）・Review（4175）をまとめて起動します。Ctrl+C でまとめて停止します。

- 起動順は固定: Launcher → Editorial → Review
- 各サービスは `GET /health` 成功後に次を起動
- ポート使用中は起動しない
- 既存の `dashboard` / `editorial:dashboard` / `aikido:review:dashboard` を子プロセスとして再利用
- 自動 restart なし（クラッシュ時は終了）
