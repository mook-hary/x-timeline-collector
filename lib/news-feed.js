...
const { validCollectionTimestamp } = require("./collection-provenance");
const fs = require("fs");
...
const { normalizeVisual, emptyVisual } = require("./visual-value");
...
// Additive v1 fields. Copy public primitives only; never spread internal metadata.
function publicVision(post) {
  const vision = post && post.vision;
  if (vision?.status !== "ok" || typeof vision.observations !== "string" ||
      !vision.observations.trim()) return null;
  return {
    status: "ok",
    observations: vision.observations,
    visibleText: typeof vision.visibleText === "string" ? vision.visibleText : null,
  };
}

function publicVisual(post) {
  try {
    // Reuse validation only, never evaluate or pass through internal extra keys.
    return normalizeVisual({ value: post?.visual?.value, roles: post?.visual?.roles });
  } catch (_error) {
    return emptyVisual();
  }
}
...
function toNewsFeedItem(post) {
  const vision = publicVision(post);
 ...
 *
 * @param {object[]} posts
 * @param {{ generatedAt?: string, collectionCompletedAt?: string|null, now?: () => Date|string, config?: object }} [options]
 */
function buildNewsFeed(posts, options = {}) {
  ...
  return {
    ...
    collectionCompletedAt: validCollectionTimestamp(options.collectionCompletedAt)
      ? options.collectionCompletedAt : null,
    scope: {
      type: SCOPE_TYPE,
...
function parseNewsFeedFromOutput(output) {
  ...
  const generated = text.match(/news-feed items=(\d+)\s+path=(\S+)/i);
  if (generated) {
    const itemCount = Number(generated[1]);
    return {
      generated: true,
  ...
  const skipped = text.match(/news-feed skipped\s+path=(\S+)/i);
  if (skipped) {
    return {
      generated: false,
      itemCount: 0,
...
function formatNewsFeedBuildLine(summary) {
  const generated = !summary || summary.generated !== false;
  if (!generated) {
    return `[build:digest-reader] news-feed skipped path=${NEWS_FEED_REL}`;
  }
  const itemCount = Number.isFinite(Number(summary && summary.itemCount))
