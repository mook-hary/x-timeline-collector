...
function createLauncherDashboardApi(options = {}) {
  ...
  function getCollectorHealth() {
    const loadFn =
      typeof options.loadLatestMorningHealthReport === "function"
        ? options.loadLatestMorningHealthReport
        : loadLatestMorningHealthReport;
    const report = loadFn(rootDir || process.cwd());
    return apiOk(collectorHealthForDashboard(report));
  }

  function getMorningPublish() {
    const loadFn =
      typeof options.loadLatestMorningHealthReport === "function"
        ? options.loadLatestMorningHealthReport
        : loadLatestMorningHealthReport;
    ...
    let scheduler = null;
    try {
      if (typeof options.statusMorningScheduler === "function") {
        scheduler = options.statusMorningScheduler({
        ...
        // Prefer plan defaults when launchd status is unavailable (non-darwin).
        try {
          scheduler = statusMorningScheduler({
            rootDir: rootDir || process.cwd(),
      ...
      scheduler = { hour: DEFAULT_HOUR, minute: DEFAULT_MINUTE };
    }
    return apiOk(morningPublishForDashboard(report, scheduler));
  }
  ...
  function getHome() {
    ...
    const morningPublish = getMorningPublish();
    ...
    return apiOk({
      ...
      collectorHealth: collector.ok ? collector.data : null,
      morningPublish: morningPublish.ok ? morningPublish.data : null,
    });
  }
