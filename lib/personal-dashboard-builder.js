...
function renderToday(today, hasHtml) {
  ...
  const link =
    hasHtml && today.href
      ? `<p><a class="btn" href="${escapeHtml(today.href)}">今日の号を読む</a></p>`
      : "";
...
function renderPick(pickCard) {
  ...
  const chips = [];
  chips.push(`<span class="chip">${escapeHtml(pickCard.entry.section)}</span>`);
  if (pickCard.entry.score != null) {
    chips.push(`<span class="chip">score ${escapeHtml(String(pickCard.entry.score))}</span>`);
  }
  ...
  <article class="card pick">
    ...
    <div class="pick__meta">${chips.join("")}</div>
    ${action}
  </article>
</section>`;
}
