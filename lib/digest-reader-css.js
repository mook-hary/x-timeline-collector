/**
 * EP-022/023/024/035 — Digest Reader CSS.
 * Morning newspaper density: spacing, hierarchy, measure. Body font-size unchanged.
 */

const DIGEST_READER_CSS = `/* Timeline Digest Reader — EP-035 */
:root {
  --ink: #1a2228;
  --ink-soft: #5a6670;
  --paper: #f2f4f1;
...
/* Morning Brief — wider measure (~70–90 JP chars), body size unchanged */
.section--brief {
  width: min(100vw - (var(--gutter) * 2), var(--max-brief));
  max-width: var(--max-brief);
  margin-inline: calc(50% - min(50vw - var(--gutter), var(--max-brief) / 2));
}
...
/* Today's Picks — denser, title-forward */
.picks-list {
  display: flex;
...
@media (max-width: 400px) {
  ...
  .section--brief {
    width: min(100% - (var(--gutter) * 2), var(--max-brief));
    margin-inline: auto;
  }
