...
/**
 * Resolve static file for dashboard apps.
 * `/shared/...` is served from sharedDir; everything else from staticDir.
 * @returns {string|null}
 */
function resolveDashboardStaticPath(staticDir, sharedDir, urlPath) {
  const raw = String(urlPath || "/");
  if (raw === "/shared" || raw.startsWith("/shared/")) {
    if (!sharedDir) return null;
    const rel = raw === "/shared" ? "/" : raw.slice("/shared".length) || "/";
    return resolveSafeStaticPath(sharedDir, rel);
  }
  return resolveSafeStaticPath(staticDir, urlPath);
}

/**
 * Stream a static file if safe; returns true when handled.
 */
function tryServeStatic(req, res, filePath) {
  const fs = require("fs");
  if (!filePath) {
