...
const {
  generateDraft: generateDraftFromKnowledge,
  generateDrafts: generateDraftsFromKnowledge,
} = require("./aikido-draft-generator");
const {
  createAikidoEditorialBridge,
} = require("./aikido-editorial-bridge");
...
function createAikidoKnowledgeStore(options = {}) {
  ...
  /**
   * Generate one Editorial-compatible draft from a knowledge id.
   * Does not save to Editorial Store.
   */
  function generateDraft(id, options = {}) {
    const safeId = validateId(id);
    const knowledge = findKnowledge(safeId);
    if (!knowledge) {
      const err = new Error(`aikido knowledge not found: ${safeId}`);
      err.code = "aikido-knowledge-not-found";
      throw err;
    }
    return generateDraftFromKnowledge(knowledge, {
      ...options,
      now: options.now != null ? options.now : nowFn,
    });
  }

  /**
   * Generate drafts for filtered knowledge list (store order after filters).
   * Does not save to Editorial Store.
   */
  function generateDrafts(options = {}) {
    ...
    return generateDraftsFromKnowledge(items, {
  ...
  function resolveBridge(callOptions = {}) {
    if (callOptions.editorialBridge) return callOptions.editorialBridge;
    if (options.editorialBridge) return options.editorialBridge;
    const store =
      callOptions.editorialStore || options.editorialStore || null;
    if (store) {
      return createAikidoEditorialBridge({ editorialStore: store });
    }
    const err = new Error(
      "editorialBridge or editorialStore is required to publish drafts"
    );
    err.code = "aikido-knowledge-no-bridge";
    throw err;
  }

  /**
   * Generate one draft from knowledge id and register via Editorial Bridge.
   * @param {string} id
   * @param {{
   *   dryRun?: boolean,
   *   allowDuplicateDraft?: boolean,
   *   editorialBridge?: object,
   ...
   * }} [callOptions]
   */
  function publishDraft(id, callOptions = {}) {
    const draft = generateDraft(id, callOptions);
    if (callOptions.dryRun === true) {
      return {
        dryRun: true,
        created: false,
        knowledgeId: draft.metadata && draft.metadata.knowledgeId,
        draft,
      };
    }
    const bridge = resolveBridge(callOptions);
    return bridge.publishDraft(draft, {
      allowDuplicateDraft: callOptions.allowDuplicateDraft,
    });
  }

  /**
   * Generate drafts (filtered) and register via Editorial Bridge in order.
   * @param {{
   *   dryRun?: boolean,
   ...
   * }} [callOptions]
   */
  function publishDrafts(callOptions = {}) {
    const drafts = generateDrafts(callOptions);
    if (callOptions.dryRun === true) {
      return {
        dryRun: true,
        drafts,
    ...
    const bridge = resolveBridge(callOptions);
    return bridge.publishDrafts(drafts, {
      continueOnError: callOptions.continueOnError,
      allowDuplicateDraft: callOptions.allowDuplicateDraft,
      // limit already applied in generateDrafts
    });
  }
