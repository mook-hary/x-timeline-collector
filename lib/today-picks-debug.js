...
function explainRejection(candidate, selected, bestScore, catCap) {
  ...
  const {
    isAttentionWithoutValue: loudThin,
  } = require("./enrichment-axes");
  if (loudThin(candidate.post)) {
    return ["Low Information / High Attention"];
  }
...
function buildTodayPicksDebug(ranked, selected, limit) {
  ...
  const selectedRows = chosen.map((item, index) => {
    ...
    return {
      ...
      legacyImportance: getImportanceOrNull(item.post),
      informationValue: getInformationValue(item.post),
      personalRelevance: getPersonalRelevance(item.post),
      impact: getImpact(item.post),
      attentionSignal: getAttentionSignal(item.post),
      source: sourceLabel(item.post),
      url: getUrl(item.post),
      signals: displaySignals(signals),
      signalKeys: [...new Set(signals)],
      selectionReason: displaySignals(signals).join(", ") || "Selected",
    };
  });
...
function formatTodayPicksDebug(debug) {
  ...
  if (debug.selected.length > 0) {
    ...
    for (const row of debug.selected) {
      ...
      lines.push(
        `   Axes: iv=${row.informationValue ?? "—"} pr=${row.personalRelevance ?? "—"} im=${row.impact ?? "—"} att=${row.attentionSignal ?? "—"}`
      );
      lines.push(`   Source: ${row.source}`);
      lines.push(`   Selection: ${row.selectionReason || "—"}`);
      lines.push("   Signals:");
