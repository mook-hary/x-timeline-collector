 ...
 * Throws on stringify / I/O failure (does not process.exit).
 */
function writeJsonAtomicOrThrow(filePath, data) {
  ...
  try {
    ...
    const err = new Error(
      `JSON serialization failed: ${filePath}\n詳細: ${error.message}`
    );
    err.code = "json-serialize";
    err.cause = error;
    throw err;
  }
  if (typeof raw !== "string") {
    const err = new Error(`JSON serialization failed: ${filePath}`);
    err.code = "json-serialize";
    throw err;
  }
 ...
 * On failure: logs and process.exit (CLI scripts).
 */
function writeJsonAtomic(filePath, data) {
  try {
    writeJsonAtomicOrThrow(filePath, data);
  } catch (error) {
