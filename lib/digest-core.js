...
const { getInformationValue } = require("./enrichment-axes");
...
/** Prefer informationValue for ranking; fall back to legacy importance. */
function getDigestRankValue(post) {
  const iv = getInformationValue(post);
  if (iv != null) return iv;
  return getImportance(post);
}
...
function getPersonalScore(post, config) {
  const rankValue = getDigestRankValue(post);
  const categoryWeight = getCategoryWeight(getDigestCategory(post), config);
  return rankValue * 10 + categoryWeight * 3;
}
...
function compareByImportanceThenDate(a, b) {
  const aImp = getDigestRankValue(a.post);
  const bImp = getDigestRankValue(b.post);
...
function buildDigestCandidate(post, index, config) {
  ...
  const informationValue = getDigestRankValue(post);
  ...
  return {
    ...
    informationValue,
    categoryWeight,
    // Prefer information value for ranking when multi-axis present (via getDigestRankValue).
    personalScore: informationValue * 10 + categoryWeight * 3,
