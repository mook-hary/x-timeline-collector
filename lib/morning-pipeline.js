...
const { resolveMorningSlaWindow } = require("./morning-deadline");
const { loadTodayPicksHistory } = require("./today-picks-history");
const { loadNewsFeedSummary } = require("./news-feed");
...
function finalizeHealth(rootDir, reportInput, deps, log) {
  const sla = resolveMorningSlaWindow({
    now: reportInput.actualStartAt || reportInput.startedAt || isoNow(deps),
  });
  const picksFreshness =
    reportInput.picksFreshness ||
    (() => {
      try {
        const snap =
          typeof deps.loadTodayPicksHistory === "function"
            ? deps.loadTodayPicksHistory(rootDir)
            : loadTodayPicksHistory(rootDir);
        return snap && snap.metrics ? snap.metrics : null;
      } catch (_error) {
        return null;
      }
    })();

  let newsFeed = reportInput.newsFeed || null;
  if (!newsFeed && reportInput.status === "SUCCESS") {
    try {
      const loadFn =
        typeof deps.loadNewsFeedSummary === "function"
          ? deps.loadNewsFeedSummary
          : loadNewsFeedSummary;
      newsFeed = loadFn(rootDir);
    } catch (_error) {
      newsFeed = null;
    }
  }
  ...
  const report = buildMorningHealthReport({
    ...reportInput,
    scheduledStartAt: reportInput.scheduledStartAt || sla.scheduledStartAt,
    actualStartAt: reportInput.actualStartAt || reportInput.startedAt,
    publishDeadline: reportInput.publishDeadline || sla.publishDeadline,
    picksFreshness,
    newsFeed,
  });
  ...
  if (
    ...
    report.warnings.includes("PIPELINE_DEADLINE_MISSED")
  ) {
    log("[morning-pipeline] WARNING: PIPELINE_DEADLINE_MISSED");
  }
  if (Array.isArray(report.warnings)) {
    for (const w of report.warnings) {
      if (String(w).startsWith("SLOW_STAGE_")) {
        log(`[morning-pipeline] WARNING: ${w}`);
      }
    }
  }
 ...
 * @param {object} [deps]
 */
async function runMorningPipeline(options, deps = {}) {
  ...
  try {
    ...
    try {
      ...
      const collectFields = extractCollectHealthFields(healthStages, error);
      finalizeHealth(
        rootDir,
        {
          ...
          status: "FAILED",
          stages: healthStages,
          collect: collectFields.collect,
          collectorHealth: collectFields.collectorHealth,
          collectorPreflight: collectFields.collectorPreflight,
          scrollRecovery: collectFields.scrollRecovery,
          homeRefresh: collectFields.homeRefresh,
          dailyScope: collectFields.dailyScope,
          newsFeed: collectFields.newsFeed,
          publish: publishInfo,
          failure: {
            stage: label,
            error: cause.message || String(cause),
            stack: cause.stack || null,
            lastStage: collectFields.lastStage || null,
          },
        },
        deps,
        log
    ...
    try {
      ...
      {
        const collectFields = extractCollectHealthFields(
          healthStages,
          morningResult
        );
        finalizeHealth(
    ...
    const health = finalizeHealth(
      rootDir,
      {
        ...
        dailyScope: collectFields.dailyScope,
        newsFeed:
          collectFields.newsFeed ||
          (morningResult && morningResult.newsFeed) ||
          null,
        publish: publishInfo,
        failure: null,
      },
