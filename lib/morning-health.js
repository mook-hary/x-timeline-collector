...
const { buildCollectorPreflight } = require("./collector-preflight");
const {
  pickScrollRecovery,
  formatScrollSummaryLabel,
  scrollRetryCount,
} = require("./collect-scroll");
const { pickHomeRefresh } = require("./collect-home-refresh");
const { pickDailyScope, buildDailyScopeSummary } = require("./daily-scope");
const {
  NEWS_FEED_REL,
...
const VISION_DEGRADED_WARNING = "VISION_DEGRADED";
...
function parseStageItemCount(stageId, output) {
  ...
  if (stageId === "vision" || stageId === "visual-value") {
    return (
      matchInt(/全投稿数:\s*(\d+)/) ??
      matchInt(/input items:\s*(\d+)/i) ??
      null
    );
  }
...
function buildMorningHealthReport(input = {}) {
  ...
  if (status === "FAILED") {
    ...
    if (f.lastStage) failure.lastStage = String(f.lastStage);
  }
  ...
  const collectorPreflight = normalizeCollectorPreflight(
    input.collectorPreflight,
    stages
  );
  const collect =
    normalizeCollectDetail(input.collect, collectorHealth, counts.collect);
  const scrollRecovery = normalizeScrollRecovery(
    input.scrollRecovery,
    collectorHealth,
    stages
  );
  const homeRefresh = normalizeHomeRefresh(
    input.homeRefresh || input,
    collectorHealth,
    stages
  );
  const dailyScope = normalizeDailyScope(
    input.dailyScope,
    collect,
    stages
  );
  const archiveTotal =
    dailyScope && dailyScope.archiveTotal != null
      ? dailyScope.archiveTotal
      : collect && collect.totalStored != null
        ? collect.totalStored
        : null;
  ...
  const newsFeed = normalizeNewsFeed(input.newsFeed, stages);
  ...
  return {
    ...
    ...(homeRefresh
      ? {
          homeRefreshed: homeRefresh.homeRefreshed,
          homeRefreshedAt: homeRefresh.homeRefreshedAt,
        }
      : {}),
    dailyScope,
    ...(archiveTotal != null ? { archiveTotal } : {}),
    newsFeed,
    publish,
    pagesDeployment,
    failure,
...
function normalizeWarnings(raw, ctx = {}) {
  ...
  for (const stage of ctx.stages || []) {
    if (stage?.id === "visual-value" && (stage.ok === false || stage.degraded === true)) push("VISUAL_VALUE_DEGRADED");
    if (
      stage &&
      stage.id === "vision" &&
      (stage.ok === false || stage.degraded === true)
    ) {
      push(VISION_DEGRADED_WARNING);
    }
  }
  return out;
}
...
function findReaderStage(stages) {
  return (stages || []).find((s) => s && s.id === "reader") || null;
}

function pickNewsFeed(input) {
  if (!input || typeof input !== "object") return null;
  return normalizeNewsFeedSummary(input.newsFeed);
}

function normalizeNewsFeed(explicit, stages) {
  return (
    normalizeNewsFeedSummary(explicit) ||
    pickNewsFeed(findReaderStage(stages)) ||
    null
  );
}
 ...
 * @returns {{ collect: object|null, collectorHealth: object|null, collectorPreflight: object|null, lastStage: string|null }}
 */
function extractCollectHealthFields(stages, errorOrResult = null) {
  ...
  const preflightRaw =
    (errorOrResult &&
      errorOrResult.collectorPreflight &&
      typeof errorOrResult.collectorPreflight === "object" &&
      errorOrResult.collectorPreflight) ||
    (stage &&
      stage.collectorPreflight &&
      typeof stage.collectorPreflight === "object" &&
      stage.collectorPreflight) ||
    null;
  const collectorPreflight = preflightRaw
    ? buildCollectorPreflight(preflightRaw)
    : null;
  const lastStage =
    (errorOrResult && errorOrResult.lastStage) ||
    (stage && stage.lastStage) ||
    (collectorPreflight && collectorPreflight.lastStage) ||
    null;
  const scrollRecovery =
    pickScrollRecovery(errorOrResult) ||
    pickScrollRecovery(collectorHealth) ||
    pickScrollRecovery(stage) ||
    null;
  const homeRefresh =
    pickHomeRefresh(errorOrResult) ||
    pickHomeRefresh(collectorHealth) ||
    pickHomeRefresh(stage) ||
    null;
  const dailyScope =
    pickDailyScope(errorOrResult) ||
    pickDailyScope({ collect, dailyScope: stage && stage.dailyScope }) ||
    null;
  const newsFeed =
    pickNewsFeed(errorOrResult) ||
    pickNewsFeed(findReaderStage(stages)) ||
    null;
...
function normalizeCollectorPreflight(explicit, stages) {
  if (explicit && typeof explicit === "object") {
    return buildCollectorPreflight(explicit);
  }
  const stage = findCollectStage(stages);
  if (
    stage &&
    ...
    return buildCollectorPreflight(stage.collectorPreflight);
  }
  return null;
}

function normalizeScrollRecovery(explicit, collectorHealth, stages) {
  return (
    pickScrollRecovery(explicit) ||
    pickScrollRecovery(collectorHealth) ||
    pickScrollRecovery(findCollectStage(stages)) ||
    null
  );
}

function normalizeHomeRefresh(explicit, collectorHealth, stages) {
  return (
    pickHomeRefresh(explicit) ||
    pickHomeRefresh(collectorHealth) ||
    pickHomeRefresh(findCollectStage(stages)) ||
    null
  );
}

function normalizeDailyScope(explicit, collect, stages) {
  return (
    buildDailyScopeSummary(explicit) ||
    pickDailyScope({ dailyScope: explicit, collect }) ||
    pickDailyScope(findCollectStage(stages)) ||
    pickDailyScope({ collect }) ||
    null
  );
}
...
function normalizeStage(raw = {}) {
  ...
  if (raw.degraded === true) stage.degraded = true;
  if (raw.fallback) stage.fallback = String(raw.fallback);
  ...
  if (raw.collectorPreflight && typeof raw.collectorPreflight === "object") {
    stage.collectorPreflight = buildCollectorPreflight(raw.collectorPreflight);
  }
  if (raw.lastStage) {
    stage.lastStage = String(raw.lastStage);
  }
  const scrollRecovery = pickScrollRecovery(raw);
  if (scrollRecovery) stage.scrollRecovery = scrollRecovery;
  const homeRefresh = pickHomeRefresh(raw);
  if (homeRefresh) {
    stage.homeRefresh = homeRefresh;
    stage.homeRefreshed = homeRefresh.homeRefreshed;
    stage.homeRefreshedAt = homeRefresh.homeRefreshedAt;
  }
  const dailyScope = pickDailyScope(raw);
  if (dailyScope) stage.dailyScope = dailyScope;
  const newsFeed = pickNewsFeed(raw);
  if (newsFeed) stage.newsFeed = newsFeed;
  return stage;
}
...
function formatMorningPipelineSummary(report, historyRelativePath) {
  ...
  if (Array.isArray(report.stages) && report.stages.length) {
    ...
    for (const stage of report.stages) {
      ...
      const tag = stageStatusTag(stage);
      lines.push(
        `${label.padEnd(12)} ${formatDurationMs(stage.durationMs || 0)}${tag}`
      );
    }
    lines.push("");
  }
  ...
  if (report.collectorHealth || report.collectorPreflight) {
    lines.push(
      formatXCollectorSummary(report.collectorHealth, report.collectorPreflight)
    );
    lines.push("");
  } else {
  ...
  if (report.dailyScope && report.dailyScope.itemCount != null) {
    lines.push("Daily Scope:");
    lines.push(`${report.dailyScope.itemCount} items`);
    lines.push("");
  }

  if (report.newsFeed && report.newsFeed.itemCount != null) {
    lines.push("News Feed:");
    lines.push(`${report.newsFeed.itemCount} items`);
    lines.push("");
  }
...
function stageStatusTag(stage) {
  if (!stage) return "";
  if (stage.degraded === true || (stage.id === "vision" && stage.ok === false)) {
    return "  [degraded]";
  }
...
function collectorHealthForDashboard(report) {
  ...
  const preflight = report.collectorPreflight || null;
  let xLogin = "—";
  let status = "unknown";
  if (health) {
  ...
  let chromeRestart = "—";
  if (preflight) {
    if (preflight.status === "recovered") cdp = "Recovered";
    else if (preflight.status === "healthy" || preflight.cdpAvailable === true) {
      cdp = "OK";
    } else {
      cdp = "Failed";
    }
    chromeRestart = preflight.chromeRestarted ? "Yes" : "No";
  }
  const collectStage = findCollectStage(report.stages);
  const scrollRecovery =
    report.scrollRecovery ||
    pickScrollRecovery(health) ||
    pickScrollRecovery(collectStage);
  const scrollLabel = formatScrollSummaryLabel(scrollRecovery);
  return {
    xLogin,
    ...
    collectorPreflight: preflight,
    cdp,
    chromeRestart,
    scroll: scrollLabel || "—",
    scrollRetry:
      scrollRecovery && scrollRetryCount(scrollRecovery) > 0
        ? scrollRetryCount(scrollRecovery)
        : scrollLabel
          ? 0
          : "—",
    lastStage:
      (collectStage && collectStage.lastStage) ||
      (preflight && preflight.lastStage) ||
      null,
    historyFile: report._historyFile || null,
  };
}
