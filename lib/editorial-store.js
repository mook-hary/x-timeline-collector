...
function normalizeItem(input, { isCreate, existing, now } = {}) {
  ...
  let id;
  if (isCreate === true) {
    id = raw.id != null && String(raw.id).trim() ? validateId(raw.id) : generateId();
  } else {
  ...
  return {
    ...
    tags: normalizeTags(merged.tags != null ? merged.tags : existing && existing.tags),
...
function createEditorialStore(options = {}) {
  ...
  const storeDir = resolveStoreDir(rootDir);
