...
function statusMorningScheduler(options = {}, deps = {}) {
  ...
  return {
    ...
    publishDeadlineHour: DEFAULT_PUBLISH_DEADLINE_HOUR,
    publishDeadlineMinute: DEFAULT_PUBLISH_DEADLINE_MINUTE,
    publishDeadline: `${String(DEFAULT_PUBLISH_DEADLINE_HOUR).padStart(2, "0")}:${String(DEFAULT_PUBLISH_DEADLINE_MINUTE).padStart(2, "0")}`,
...
function formatStatusReport(status) {
  ...
  lines.push(
    `Deadline:  ${status.publishDeadline || "07:00"} (publish complete by)`
  );
...
Usage:
  npm run scheduler:install -- [--hour 3] [--minute 0] [--timezone Asia/Tokyo]
  npm run scheduler:uninstall
  npm run scheduler:status

Default: daily 03:00 Asia/Tokyo start → npm run morning (via morning-pipeline.js)
Publish SLA: complete GitHub Pages publish by 07:00 local time
