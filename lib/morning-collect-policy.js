...
function isCollectCdpFailure(result) {
  ...
  const out = combinedOutput(result);
  if (/SCROLL_TIMEOUT|HOME_REFRESH_TIMEOUT/.test(out)) return false;
  if (
    /Chrome への接続に失敗|接続先:.*9222|connectToChrome|ECONNREFUSED/i.test(
      out
    ) ||
    /CDP_NOT_AVAILABLE|CDP_CONNECT_TIMEOUT|CDP_CONTEXT_TIMEOUT|X_HOME_UNAVAILABLE/.test(
      out
    )
  ) {
    return true;
