...
const { extractLinkedResourceKey } = require("./digest-core");
const { isAttentionWithoutValue } = require("./enrichment-axes");
...
function getDomainKey(post) {
  // Prefer external article host when the post quotes a linked story.
  const linked = getLinkedArticleKey(post);
  if (linked.startsWith("link:")) {
    const host = linked.slice(5).split("/")[0];
    if (host) return host;
  }
...
function selectTodayPicksFromRanked(ranked, limit) {
  ...
  function conflictsDuplicate(candidate, selected) {
    const candidateLink = getLinkedArticleKey(candidate.post);
    for (const item of selected) {
      if (isExactDuplicate(candidate.post, item.post)) return true;
      if (isNearDuplicate(candidate.post, item.post)) return true;
      // Same external article quoted by different X posts → one primary pick.
      if (
        candidateLink &&
        candidateLink === getLinkedArticleKey(item.post)
      ) {
        return true;
      }
    }
  ...
  function tryPass(pass) {
    ...
    for (const candidate of list) {
      ...
      // Do not fill primary slots with attention-only low-value posts.
      if (
        pass <= 2 &&
        isAttentionWithoutValue(candidate.post)
      ) {
        continue;
      }
      ...
      if (pass === 1) {
        ...
        // Still avoid attention-without-value unless nothing else remains.
        if (!qualityOk) continue;
        if (
          isAttentionWithoutValue(candidate.post) &&
          list.some(
            (other) =>
              !isAttentionWithoutValue(other.post) &&
              meetsQualityFloor(other.editorialScore, bestScore)
          )
        ) {
          continue;
        }
      ...
      selected.push({
        ...candidate,
        selectionPass: pass,
        selectionSignals: buildAcceptSignals({
          pass,
          catCount,
          authorCount,
          domainCount,
...
function buildAcceptSignals({
  ...
  const signals = ["unique-topic"];
  if (pass > 1) signals.push("fallback-selection");
  if (catCount === 0) signals.push("category-balance");
  if (selectedSoFar > 0 && author && authorCount === 0) {
    signals.push("source-diversity");
  }
  if (
    selectedSoFar > 0 &&
    domain &&
 ...
 * Debug payload is separate and never written to Reader / history.
 *
 ...
 * @returns {object[]}
 */
function selectTodayPicks(filteredPosts, topN, contextFn = null, options = {}) {
  const { picks, debug, freshness } = selectTodayPicksDetailed(
    filteredPosts,
    topN,
    contextFn,
    options
  );
  const { maybeLogTodayPicksDebug } = require("./today-picks-debug");
  maybeLogTodayPicksDebug(debug);
  if (freshness && process.env.DEBUG_TODAY_PICKS === "true") {
    const { formatPicksFreshnessLine } = require("./today-picks-freshness");
    const line = formatPicksFreshnessLine(freshness.metrics);
...
/**
 * Same as selectTodayPicks but returns { picks, debug, freshness } without logging.
 *
 * @returns {{ picks: object[], debug: object|null, freshness: object|null }}
 */
function selectTodayPicksDetailed(
  filteredPosts,
  topN,
  contextFn = null,
  options = {}
  ...
  let ranked = rankedPosts.map((item, index) => ({
    post: item.post,
  ...
  let freshness = null;
  const previousPicks = Array.isArray(options.previousPicks)
    ? options.previousPicks
    : null;
  if (previousPicks && previousPicks.length) {
    const { applyPreviousDayPenalties } = require("./today-picks-freshness");
    const applied = applyPreviousDayPenalties(ranked, previousPicks);
    ranked = applied.ranked;
    // Re-sort after ephemeral penalty so demoted repeats lose primary slots.
    ...
    freshness = {
      ...
      previousCount: previousPicks.length,
    };
  }
  ...
  const picks = selected.map((item) => {
    ...
    return {
      ...
      _previousDayPenalty: item.previousDayPenalty || 0,
    };
  });

  if (freshness) {
    const { computePicksFreshnessMetrics } = require("./today-picks-freshness");
    freshness.metrics = computePicksFreshnessMetrics(
      selected.map((s) => s.post),
      previousPicks
    );
  }

  const { buildTodayPicksDebug } = require("./today-picks-debug");
  const debug = buildTodayPicksDebug(ranked, selected, limit);

  return { picks, debug, freshness };
}
