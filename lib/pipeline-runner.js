...
function runPipeline(options = {}, hooks = {}) {
  ...
  try {
    ...
    // 12) writer-batch — 1 selected Story = 1 article (+ legacy primary for AR/DE)
    start("writer-batch");
    let markdown;
    const articlePath = path.join(workDir, "article.md");
    let articlesManifest = null;
    {
      const { runWriterBatch } = require("./writer-batch");
      ...
      if (
        ...
        log("[pipeline] writer-batch: skipped (edition selected 0 stories)");
        const emptyBatch = runWriterBatch({
          selectionResult: writerSelectionResult,
          brief: JSON.parse(fs.readFileSync(briefPath, "utf8")),
          plan: JSON.parse(fs.readFileSync(planPath, "utf8")),
          outputDir: articlesDir,
          manifestPath,
          legacyPrimaryPath: articlePath,
          originalStoriesInput: fs.existsSync(storiesPath)
            ? JSON.parse(fs.readFileSync(storiesPath, "utf8"))
            : { stories: [] },
        });
        articlesManifest = emptyBatch.manifest;
        markdown = "";
        log(
          `[pipeline] writer-batch: requested=0 generated=0 failed=0 primary=(none)`
        );
        mark("writer-batch");
      } else {
        const briefPayload = JSON.parse(fs.readFileSync(briefPath, "utf8"));
        const planPayload = JSON.parse(fs.readFileSync(planPath, "utf8"));
        const originalStories = fs.existsSync(storiesPath)
          ? JSON.parse(fs.readFileSync(storiesPath, "utf8"))
          : { stories: [] };
        ...
        const batch = runWriterBatch({
          ...
          manifestPath,
          legacyPrimaryPath: articlePath,
          originalStoriesInput: originalStories,
        });
        articlesManifest = batch.manifest;
        markdown = batch.legacyPrimaryMarkdown || "";
        log(
          `[pipeline] writer-batch: requested=${batch.manifest.summary.requestedCount} generated=${batch.manifest.summary.generatedCount} failed=${batch.manifest.summary.failedCount} primary=${batch.legacyPrimaryArticlePath || "(none)"} manifest=${manifestPath}`
        );
        mark("writer-batch");
      }
    }

    // 13) article-report-batch — 1 generated article = 1 Report (+ legacy primary)
    start("article-report-batch");
    let report = null;
    let reportPath = null;
    {
      const { runArticleReportBatch } = require("./article-report-batch");
      const briefPayload = JSON.parse(fs.readFileSync(briefPath, "utf8"));
      const planPayload = JSON.parse(fs.readFileSync(planPath, "utf8"));
      const manifestPath = path.join(workDir, "articles-manifest.json");
      const reportsDir = path.join(workDir, "article-reports");
      const legacyReportWorkPath = path.join(workDir, "article-report.json");

      let manifestPayload = articlesManifest;
      if (!manifestPayload && fs.existsSync(manifestPath)) {
        manifestPayload = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
      }
      if (!manifestPayload) {
        manifestPayload = {
          version: "1.0",
      ...
      const batchReport = runArticleReportBatch({
        manifest: manifestPayload,
        brief: briefPayload,
        plan: planPayload,
        outputDir: reportsDir,
        manifestPath,
        legacyReportPath: reportPath || legacyReportWorkPath,
        articlesDir: path.join(workDir, "articles"),
        confidenceThreshold:
          options.confidenceThreshold != null
            ? options.confidenceThreshold
            : 50,
        now: briefPayload.generatedAt || "2026-07-21T00:00:00.000Z",
      });

      articlesManifest = batchReport.manifest;
      report = batchReport.legacyPrimaryReport;
      if (!reportPath && report) {
        // already written to workDir by batch
      }

      const rs = batchReport.reportSummary || {};
      log(
        `[pipeline] article-report-batch: requested=${rs.requestedCount || 0} generated=${rs.generatedCount || 0} failed=${rs.failedCount || 0} skipped=${rs.skippedCount || 0} primary=${batchReport.legacyPrimaryReportPath || "(none)"}`
      );

      if (report && report.reviewSummary && report.reviewSummary.status === "fail") {
        const err = new Error(
          `Article Report reviewSummary.status=fail（errorCount=${report.reviewSummary.errorCount}）`
        );
        err.step = "article-report-batch";
        err.report = report;
        throw err;
      }

      // Keep workDir copy even when --report-output is set
      if (report && reportPath) {
        fs.writeFileSync(
          legacyReportWorkPath,
          `${JSON.stringify(report, null, 2)}\n`,
          "utf8"
        );
      }

      mark("article-report-batch");
    }

    // 14) daily-edition-builder — Manifest 全件から section 紙面を構築
    let dailyEditionDoc = null;
    let dailyEditionPath = null;
    let dailyEditionAutoManifestPath = null;
    {
      start("daily-edition-builder");
      const {
        buildDailyEditionFromArticlesManifest,
      } = require("./daily-edition-builder");
      const articlesManifestPath = path.join(workDir, "articles-manifest.json");
      dailyEditionPath = path.join(workDir, "daily-edition.json");
      dailyEditionAutoManifestPath = path.join(
        workDir,
        "daily-edition-auto-manifest.json"
      );

      let manifestPayload = articlesManifest;
      if (!manifestPayload && fs.existsSync(articlesManifestPath)) {
        manifestPayload = JSON.parse(
          fs.readFileSync(articlesManifestPath, "utf8")
        );
      }
      if (!manifestPayload) {
        manifestPayload = {
        ...
        writeJsonAtomic(articlesManifestPath, manifestPayload);
      }

      const built = buildDailyEditionFromArticlesManifest({
        manifest: articlesManifestPath,
        outputPath: dailyEditionPath,
        legacyDeManifestPath: dailyEditionAutoManifestPath,
        date:
          (range && range.to) ||
          new Date().toISOString().slice(0, 10),
      });
      dailyEditionDoc = built.editionDoc;
      const sc = dailyEditionDoc.summary.sectionCounts || {};
      log(
        `[pipeline] daily-edition-builder: articles=${dailyEditionDoc.summary.articleCount} top=${sc.top || 0} secondary=${sc.secondary || 0} brief=${sc.brief || 0} warnings=${dailyEditionDoc.summary.warningsCount || 0} out=${dailyEditionPath}`
      );
      mark("daily-edition-builder");
    }

    // 15) daily-edition-html — human-readable newspaper preview
    let dailyEditionHtmlPath = null;
    let dailyEditionCssPath = null;
    {
      start("daily-edition-html");
      const { buildDailyEditionHtml } = require("./daily-edition-html");
      const editionHtmlDir = path.join(rootDir, "output", "edition");
      const htmlBuilt = buildDailyEditionHtml({
        edition: dailyEditionPath,
        workRoot: workDir,
        outputDir: editionHtmlDir,
        title: "Daily Edition",
        date:
          (range && range.to) ||
          new Date().toISOString().slice(0, 10),
      });
      dailyEditionHtmlPath = htmlBuilt.htmlPath;
      dailyEditionCssPath = htmlBuilt.cssPath;
      log(
        `[pipeline] daily-edition-html: articles=${htmlBuilt.articleCount} warnings=${htmlBuilt.warnings.length} out=${dailyEditionHtmlPath}`
      );
      mark("daily-edition-html");
    }

    // 16) edition-archive-builder — date-keyed archive of finalized edition
    let editionArchiveDir = null;
    let editionArchiveManifest = null;
    {
      start("edition-archive-builder");
      const { buildEditionArchive } = require("./edition-archive-builder");
      const archived = buildEditionArchive({
        edition: dailyEditionPath,
        workRoot: workDir,
        htmlPath: dailyEditionHtmlPath,
        cssPath: dailyEditionCssPath,
        outputRoot: path.join(rootDir, "output"),
        editionDate:
          (dailyEditionDoc && dailyEditionDoc.editionDate) ||
          (range && range.to) ||
          null,
        now: new Date(),
      });
      editionArchiveDir = archived.archiveDir;
      editionArchiveManifest = archived.manifest;
      const sum = archived.manifest.summary || {};
      log(
        `[pipeline] edition-archive-builder: edition=${archived.editionId} articles=${sum.articleCount || 0} reports=${sum.reportCount || 0} warnings=${sum.warningsCount || 0}`
      );
      mark("edition-archive-builder");
    }

    // 17) personal-dashboard-builder — morning entry screen
    let dashboardHtmlPath = null;
    let dashboardCssPath = null;
    {
      start("personal-dashboard-builder");
      const { buildPersonalDashboard } = require("./personal-dashboard-builder");
      const dash = buildPersonalDashboard({
        edition: dailyEditionPath,
        workRoot: workDir,
        outputDir: path.join(rootDir, "output"),
        archiveRoot: path.join(rootDir, "output", "archive"),
        latestHtmlPath: dailyEditionHtmlPath,
        requireEdition: true,
      });
      dashboardHtmlPath = dash.htmlPath;
      dashboardCssPath = dash.cssPath;
      const s = dash.summary || {};
      log(
        `[pipeline] personal-dashboard-builder: today=${s.todayCount || 0} readNext=${s.readNextCount || 0} topics=${s.topicCount || 0} archives=${s.archiveCount || 0} warnings=${s.warningsCount || 0}`
      );
      mark("personal-dashboard-builder");
    }

    // 18) site-builder — GitHub Pages / PWA static site
    let siteRootPath = null;
    {
      start("site-builder");
      const { buildSite } = require("./site-builder");
      const siteBuilt = buildSite({
        rootDir,
        outputRoot: path.join(rootDir, "output"),
        siteRoot: path.join(rootDir, "site"),
      });
      siteRootPath = siteBuilt.siteRoot;
      const ss = siteBuilt.summary || {};
      log(
        `[pipeline] site-builder: edition=${ss.hasEdition ? 1 : 0} archives=${ss.archiveCount || 0} warnings=${ss.warningsCount || 0} out=${siteRootPath}`
      );
      mark("site-builder");
    }

    // 19) daily-edition (optional: only when --daily-manifest is set)
    ...
    return {
      ...
      dailyEditionDoc,
      dailyEditionPath,
      dailyEditionAutoManifestPath,
      dailyEditionHtmlPath,
      dailyEditionCssPath,
      editionArchiveDir,
      editionArchiveManifest,
      dashboardHtmlPath,
      dashboardCssPath,
      siteRootPath,
      dailyEdition,
