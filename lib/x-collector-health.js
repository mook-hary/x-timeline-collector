...
function buildCollectorHealth(input = {}) {
  ...
  const scrollRecovery = pickScrollRecovery(input);
  if (scrollRecovery) out.scrollRecovery = scrollRecovery;
  const homeRefresh = pickHomeRefresh(input);
  if (homeRefresh) {
    out.homeRefreshed = homeRefresh.homeRefreshed;
    out.homeRefreshedAt = homeRefresh.homeRefreshedAt;
    out.homeRefresh = homeRefresh;
  }
 ...
 */
function formatCdpSummaryLabel(preflight) {
  if (!preflight || typeof preflight !== "object") return null;
  if (preflight.status === "recovered") return "Recovered";
  if (preflight.status === "healthy" || preflight.cdpAvailable === true) {
    return "OK";
  }
  return "FAILED";
}

function formatCollectSummaryLabel(health) {
  if (!health || typeof health !== "object") return "FAILED";
...
function appendScrollSummary(lines, health) {
  ...
  lines.push(`Scroll: ${scrollLabel}`);
  if (scrollLabel === "Recovered" || scrollRetryCount(scrollRecovery) > 0) {
    lines.push(`Retry: ${scrollRetryCount(scrollRecovery)}`);
  }
}
 ...
 * @param {object|null} [preflight]
 */
function formatXCollectorSummary(health, preflight = null) {
  const lines = ["X Collector"];
  const cdpLabel = formatCdpSummaryLabel(preflight);
  if (cdpLabel) {
    lines.push(`CDP: ${cdpLabel}`);
    lines.push(
      `Chrome Restart: ${preflight && preflight.chromeRestarted ? "Yes" : "No"}`
    );
  }
  if (!health || typeof health !== "object") {
    lines.push("Login: —");
    if (preflight) lines.push(`Collect: ${formatCollectSummaryLabel(health)}`);
    return lines.join("\n");
  }
  if (health.authenticated === false || health.error === AUTH_ERROR) {
    lines.push("Login: FAILED");
    lines.push(`Reason: ${health.error || AUTH_ERROR}`);
    lines.push(`Collect: ${formatCollectSummaryLabel(health)}`);
    appendScrollSummary(lines, health);
    return lines.join("\n");
  }
  lines.push("Login: OK");
