...
/**
 * Exact public GitHub Pages artifacts under otherwise-private `output/`.
 * Directory-wide exceptions are intentionally not supported.
 */
const ALLOWED_PUBLIC_OUTPUT_FILES = new Set([
...
function isSafeApiKeyValue(value) {
  ...
  if (!raw) return true;
  // Redaction markers win even when wrapped in quotes / code trailers.
  if (/\[REDACTED\]/i.test(raw)) return true;
  if (raw === "REDACTED") return true;
  // Strip wrapping quotes and common JS/source trailers from line scanners.
  // Do not strip `]` — it appears inside `[REDACTED]`.
  let v = raw.replace(/^["']+/, "").replace(/["');,]+$/g, "");
  if (!v) return true;
  if (v === PLACEHOLDER_API_KEY) return true;
  if (/^your_/i.test(v)) return true;
  if (/^<.*>$/.test(v)) return true;
  if (/^\.+/.test(v)) return true;
  // Env / config references — not hardcoded secrets.
  if (/^(process\.env\.|env\.)[A-Za-z0-9_]+$/i.test(v)) return true;
...
/**
 * True when OPENAI_API_KEY appears only as a name / redaction / env reference.
 * Literal assignments of live-looking values remain unsafe via isSafeApiKeyValue.
 */
function isBenignOpenAiKeyMention(line) {
  const s = String(line || "");
  if (!/OPENAI_API_KEY/i.test(s)) return false;
  // Redaction / sanitizer replacement targets.
  if (/OPENAI_API_KEY\s*=\s*\[REDACTED\]/i.test(s)) return true;
  if (/OPENAI_API_KEY\[REDACTED\]/i.test(s)) return true;
  // Env property access (no literal secret on RHS of KEY=).
  if (/process\.env\.OPENAI_API_KEY\b/.test(s) && !OPENAI_ASSIGN_RE.test(s)) {
    return true;
  }
  if (/\benv\.OPENAI_API_KEY\b/.test(s) && !OPENAI_ASSIGN_RE.test(s)) {
    return true;
  }
  return false;
}
  ...
  for (const file of trackedFiles) {
    ...
    for (const prefix of FORBIDDEN_TRACKED_PREFIXES) {
      if (normalized === prefix.slice(0, -1) || normalized.startsWith(prefix)) {
        if (
          prefix === "output/" &&
          ALLOWED_PUBLIC_OUTPUT_FILES.has(normalized)
        ) {
          continue;
        }
        pushFinding(findings, {
...
function scanLineSecrets(filePath, line, lineNo, findings) {
  ...
  if (envMatch && !isSafeApiKeyValue(envMatch[1]) && !isBenignOpenAiKeyMention(line)) {
    pushFinding(findings, {
