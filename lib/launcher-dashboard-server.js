...
function createLauncherDashboardServer(options = {}) {
  ...
  const sharedDir =
    options.sharedDir || path.join(rootDir, "shared");
  const api =
    options.api ||
    createLauncherDashboardApi({
      rootDir,
  ...
  async function handleApi(req, res, pathname) {
    try {
      ...
      if (req.method === "GET" && pathname === "/api/collector-health") {
        const result =
          typeof api.getCollectorHealth === "function"
      ...
      if (req.method === "GET" && pathname === "/api/morning-publish") {
        const result =
          typeof api.getMorningPublish === "function"
            ? api.getMorningPublish()
            : { ok: true, data: null };
        return sendJson(res, result.ok ? 200 : result.status || 400, result);
      }
  ...
  function handleStatic(req, res, pathname) {
    ...
    const filePath = resolveDashboardStaticPath(staticDir, sharedDir, pathname);
    tryServeStatic(req, res, filePath);
  }
