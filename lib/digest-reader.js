 ...
 * Reuses digest.js filters (buildDigest). Today's Picks via selectTodayPicks.
 * No AI. Writes only under output/digest-reader/.
 */

const { readArtifactSnapshot, parseSnapshot } = require("./collection-provenance");
const fs = require("fs");
...
const { selectTodayPicks, selectTodayPicksDetailed } = require("./today-picks");
const {
  loadPreviousTodayPicks,
  saveTodayPicksHistory,
} = require("./today-picks-history");
const { buildTodayBrief } = require("./today-brief");
const {
  createBuildProfiler,
  isReaderProfileEnabled,
} = require("./build-profiler");
const {
  HISTORY_REL,
  loadHistory,
 ...
 * @returns {object[]}
 */
function selectTodaysPicks(filteredPosts, topN, config = null, options = {}) {
  const limit = Math.max(0, Number(topN) || DEFAULT_TOP);
  return selectTodayPicks(
    filteredPosts,
    limit,
    (post) => {
      if (!config) return {};
...
/**
 * More News — category headings + primary cards + expandable overflow.
 * Overflow posts are rendered hidden; each category button expands independently.
 */
function renderMoreNews(digest) {
  ...
  const sections = groups
    .map((group, index) => {
      ...
      const morePosts = Array.isArray(group.morePosts) ? group.morePosts : [];
      const remaining = Math.max(
        0,
        morePosts.length > 0
          ? morePosts.length
          : Number(group.count || 0) - posts.length
      );
      const cards =
        posts.length === 0
          ? `<p class="empty">このカテゴリに表示投稿はありません。</p>`
          : posts.map((p) => renderCard(p)).join("\n");
      const extraId = `more-cat-extra-${index}`;
      const extraCards =
        morePosts.length > 0
          ? morePosts.map((p) => renderCard(p)).join("\n")
          : "";
      const extraPanel =
        remaining > 0 && morePosts.length > 0
          ? `<div class="more-cat__extra is-collapsed" id="${extraId}" hidden data-more-panel="${index}">
  ${extraCards}
</div>`
          : "";
      const more =
        remaining > 0 && morePosts.length > 0
          ? `<p class="more-read-wrap"><button type="button" class="more-read" data-more-target="${extraId}" data-more-count="${escapeHtml(
              String(remaining)
            )}" aria-controls="${extraId}" aria-expanded="false">${escapeHtml(
              moreReadOpenLabel(remaining)
            )}</button></p>`
          : remaining > 0
            ? `<p class="more-read-wrap"><span class="more-read more-read--static">さらに${escapeHtml(
                String(remaining)
              )}件</span></p>`
            : "";
...
const MORE_READ_CLOSE_LABEL = "閉じる";

function moreReadOpenLabel(count) {
  return `さらに${Number(count)}件読む →`;
}

/**
 * Pure toggle state for「さらに読む」↔「閉じる」.
 * Count is preserved from the initial remaining value.
 */
function toggleMoreReadState(isExpanded, count) {
  if (isExpanded) {
    return {
      expanded: false,
      label: moreReadOpenLabel(count),
      panelHidden: true,
    };
  }
  return {
    expanded: true,
    label: MORE_READ_CLOSE_LABEL,
    panelHidden: false,
  };
}

/** Apply open/close toggle to a button + panel (DOM-like objects). */
function applyMoreReadToggle(btn, panel) {
  if (!btn || !panel) return null;
  const count = btn.getAttribute("data-more-count") || "0";
  const isExpanded = btn.getAttribute("aria-expanded") === "true";
  const next = toggleMoreReadState(isExpanded, count);
  panel.hidden = next.panelHidden;
  if (next.panelHidden) {
    if (panel.classList && typeof panel.classList.add === "function") {
      panel.classList.add("is-collapsed");
    }
  } else if (panel.classList && typeof panel.classList.remove === "function") {
...
/** Inline toggle handler for per-category「さらに読む」buttons. */
function renderMoreReadScript() {
  return `<script id="digest-reader-more-script">
(function () {
  function moreReadOpenLabel(count) {
    return "さらに" + String(count) + "件読む →";
  }
  function toggleMore(btn) {
    ...
    if (!panel) return;
    var count = btn.getAttribute("data-more-count") || "0";
    var open = btn.getAttribute("aria-expanded") === "true";
    if (open) {
      panel.hidden = true;
      panel.classList.add("is-collapsed");
      btn.setAttribute("aria-expanded", "false");
      btn.textContent = moreReadOpenLabel(count);
    } else {
      panel.hidden = false;
      panel.classList.remove("is-collapsed");
  ...
  document.addEventListener("click", function (event) {
    var btn = event.target && event.target.closest
      ? event.target.closest("button.more-read[data-more-target]")
      : null;
    if (!btn) return;
    toggleMore(btn);
  });
...
function buildHtmlDocument(digest, usageDashboard, parts = {}) {
  ...
  const categoryDigestHtml =
    parts.categoryDigestHtml != null
      ? parts.categoryDigestHtml
...
${Number(digest.total) === 0 ? "" : renderMoreReadScript()}
</body>
</html>
`;
}
 ...
 * @param {boolean} [options.profile] — override READER_PROFILE
 * @param {NodeJS.WritableStream} [options.profileStream]
 */
function buildDigestReader(options = {}) {
  const profileEnabled =
    options.profile != null
      ? Boolean(options.profile)
      : isReaderProfileEnabled(options.env || process.env);
  const profiler = createBuildProfiler({ enabled: profileEnabled });
  profiler.startWall();
  ...
  const { config, range, posts } = profiler.measure("Load Data", () => {
    const loadedConfig = options.config || loadConfig();
    const loadedRange = resolveLocalDateRange(digestOptions, fail);
    let loadedPosts;
    if (options.posts) {
      loadedPosts = options.posts;
    } else {
      const snapshot = readArtifactSnapshot(inputPath);
      loadedPosts = parseSnapshot(snapshot);
      collectionCompletedAt = snapshot.provenance?.collectionCompletedAt ?? null;
      if (!snapshot.provenance) {
        (options.provenanceWarn || console.warn)(`[collection-provenance] unverified: ${snapshot.reason}`);
      }
    }
    return {
      config: loadedConfig,
  ...
  const { digest, selection } = profiler.measure("Editorial", () => {
    const built = buildDigest(posts, digestOptions, range, config);
    const sel = buildDigestSelection(posts, digestOptions, range, config);
    return { digest: built, selection: sel };
  });

  digest.todaysPicks = profiler.measure("Today's Picks", () => {
    const previous = loadPreviousTodayPicks(rootDir, {
      now: options.now,
    });
    const previousPicks = previous && Array.isArray(previous.picks)
      ? previous.picks
      : [];
    const detailed = selectTodayPicksDetailed(
      selection.filtered,
      digestOptions.top,
      (post) => ({ personalScore: getPersonalScore(post, config) }),
      { previousPicks }
    );
    digest.picksFreshness = detailed.freshness && detailed.freshness.metrics
      ? detailed.freshness.metrics
      : null;
    try {
      saveTodayPicksHistory(rootDir, detailed.picks, {
        now: options.now,
        metrics: digest.picksFreshness,
      });
    } catch (_error) {
      // Soft-fail: Reader build must not die on history I/O.
    }
    return detailed.picks;
  });

  digest.todayBrief = profiler.measure("Morning Brief", () =>
    buildTodayBrief(selection.filtered, digest.todaysPicks)
  );

  const usageDashboard = profiler.measure("Load Data", () =>
    loadUsageDashboardForReader(rootDir, options)
  );

  const categoryDigestHtml = profiler.measure("Category Digest", () =>
    Number(digest.total) === 0 ? "" : renderCategoryDigest(digest)
  );

  const html = profiler.measure("HTML Render", () =>
    buildHtmlDocument(digest, usageDashboard, {
      categoryDigestHtml,
    })
  );
  ...
  const feedPath = newsFeedPath(rootDir, outputDir);
  let newsFeedSummary;
  profiler.measure("Write File", () => {
    ...
    const exportFeed =
      options.exportNewsFeed === true ||
      (options.exportNewsFeed !== false &&
        (Boolean(options.posts) ||
          path.basename(inputPath) === "daily-enriched.json"));
    if (!exportFeed) {
      newsFeedSummary = {
        generated: false,
        itemCount: 0,
        path: path.join("output", "digest-reader", "news-feed.json"),
      };
      return;
    }
    const newsFeedDoc = buildNewsFeed(posts, {
      generatedAt: digest.generatedAt,
      collectionCompletedAt,
      now: options.now,
      config,
    });
    newsFeedSummary = writeNewsFeed(feedPath, newsFeedDoc);
  });

  profiler.endWall();
  if (profileEnabled) {
    profiler.report(options.profileStream || process.stderr);
  }
  ...
  return {
    ...
    htmlPath,
    cssPath,
    newsFeedPath: feedPath,
    newsFeed: newsFeedSummary,
    digest,
    profile: profiler.toJSON(),
    summary: {
      total: digest.total,
      ...
      picksFreshness: digest.picksFreshness || null,
      newsFeedItems: newsFeedSummary ? newsFeedSummary.itemCount : 0,
    },
  };
}
