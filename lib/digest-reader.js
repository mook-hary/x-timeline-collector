...
function renderPickCard(post) {
  ...
  if (hasSummary) {
    parts.push(`<p class="card__summary">${escapeHtml(summary)}</p>`);
  }
  if (showBody) {
    parts.push(`<p class="card__body">${escapeHtml(text)}</p>`);
  }
  if (reason) {
    parts.push(`<p class="card__reason">${escapeHtml(reason)}</p>`);
  }
  if (importance != null) {
...
function renderTodayBrief(digest) {
  ...
  return `<section class="section" aria-labelledby="brief-label">
  <h2 class="section__label" id="brief-label">Today's Brief</h2>
  <div class="brief">
    ${listHtml}
  </div>
</section>`;
