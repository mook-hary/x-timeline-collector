...
function renderMoreNews(digest) {
  ...
  ${extraCards}
      ...
      const more =
        remaining > 0 && morePosts.length > 0
          ? `<p class="more-read-wrap"><button type="button" class="more-read" data-more-target="${extraId}" aria-controls="${extraId}" aria-expanded="false">さらに${escapeHtml(
              String(remaining)
            )}件読む →</button></p>`
          : remaining > 0
...
/** Inline expand handler for per-category「さらに読む」buttons. */
...
(function () {
  function expandMore(btn) {
    ...
    var wrap = btn.closest(".more-read-wrap");
