...
const { selectTodayPicks } = require("./today-picks");
...
function selectTodaysPicks(filteredPosts, topN, config = null) {
  ...
  return selectTodayPicks(filteredPosts, limit, (post) => {
    if (!config) return {};
...
function buildDigestReader(options = {}) {
  ...
  digest.todaysPicks = profiler.measure("Today's Picks", () =>
    selectTodaysPicks(selection.filtered, digestOptions.top, config)
  );
