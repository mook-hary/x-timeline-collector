...
} = require("./aikido-knowledge-extractor");
...
function createAikidoSourceIntake(options = {}) {
  ...
  function resolveExtractor(callOptions = {}) {
    if (callOptions.extractor) return callOptions.extractor;
    if (options.extractor) return options.extractor;
    const provider = callOptions.provider || options.provider;
    return createAikidoKnowledgeExtractor({ provider });
  }

  /**
   * Extract Knowledge candidates from one source (no status change / no save).
   */
  function extractKnowledge(id, callOptions = {}) {
    const safeId = validateId(id);
    const source = findSource(safeId);
    if (!source) {
      const err = new Error(`aikido source not found: ${safeId}`);
      err.code = "aikido-source-not-found";
      throw err;
    }
    const extractor = resolveExtractor(callOptions);
    return extractor.extractFromSource(source, callOptions);
  }

  /**
   * Batch extract from listed sources (filters via listSources).
   * Does not change Source status; does not create Knowledge.
   */
  function extractKnowledgeBatch(callOptions = {}) {
    const sources = listSources({
      sourceType: callOptions.sourceType,
      status: callOptions.status,
      language: callOptions.language,
      tag: callOptions.tag,
      author: callOptions.author,
      publisher: callOptions.publisher,
      hasUrl: callOptions.hasUrl,
      hasRawText: callOptions.hasRawText,
    });
    const extractor = resolveExtractor(callOptions);
    return extractor.extractFromSources(sources, callOptions);
  }

  function resolveCandidateReview(callOptions = {}) {
    if (callOptions.candidateReview) return callOptions.candidateReview;
    if (options.candidateReview) return options.candidateReview;
    return null;
  }

  /**
   * Extract candidates and persist as Reviews.
   * Does not approve, convert, or change Source status.
   */
  function extractAndCreateReviews(id, callOptions = {}) {
    const review = resolveCandidateReview(callOptions);
    if (!review || typeof review.createReviews !== "function") {
      const err = new Error(
        "candidateReview with createReviews() is required (inject via options)"
      );
      err.code = "aikido-source-review";
      throw err;
    }
    const extraction = extractKnowledge(id, callOptions);
    return review.createReviews(extraction, callOptions);
  }

  /**
   * Batch extract + create reviews. Does not mutate Source status.
   */
  function extractBatchAndCreateReviews(callOptions = {}) {
    const review = resolveCandidateReview(callOptions);
    if (!review || typeof review.createReviews !== "function") {
      const err = new Error(
        "candidateReview with createReviews() is required (inject via options)"
    ...
    const batch = extractKnowledgeBatch(callOptions);
    const allReviews = [];
    const allErrors = [];
    ...
    for (const extraction of batch.results || []) {
      if (extraction.metadata && extraction.metadata.failed) {
        errorCount += 1;
        allErrors.push({
          sourceId: extraction.sourceId,
          message:
            extraction.errors && extraction.errors[0]
              ? extraction.errors[0].message
              : "extraction failed",
        });
        continue;
      }
      const created = review.createReviews(extraction, callOptions);
      allReviews.push(...created.reviews);
      allErrors.push(
        ...created.errors.map((e) => ({
          ...e,
          sourceId: extraction.sourceId,
        }))
      );
      candidateCount += created.summary.candidateCount;
      createdCount += created.summary.createdCount;
      skippedCount += created.summary.skippedCount;
      errorCount += created.summary.errorCount;
    }
  ...
  return {
    ...
    extractAndCreateReviews,
    extractBatchAndCreateReviews,
  };
}
