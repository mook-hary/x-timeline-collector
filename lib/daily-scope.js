...
const DAILY_VISION_REL = path.join("output", "daily-vision.json");
...
function morningVisionArgs() {
  return ["--apply", "--input", DAILY_ANALYZED_REL, "--output", DAILY_VISION_REL];
}

function morningVisualValueArgs() {
  return ["--apply", "--input", DAILY_VISION_REL, "--output", path.join("output", "daily-visual.json")];
}
...
function morningAnalyzeAiArgs(limit) {
  return [
    ...
    String(limit),
    "--input",
    path.join("output", "daily-visual.json"),
    "--output",
    DAILY_AI_REL,
  ];
}

function morningAnalyzeAiFallbackArgs(limit) {
  return [
    "--limit",
    String(limit),
    "--input",
